...
int block_read(struct block_device *blk, void *buf, sector_t block, blkcnt_t num_blocks)
{
	if (fuzz_start)
    {
        if(((block + num_blocks) * 512) <= payload_buffer->size)
        {
            memcpy(buf, payload_buffer->data + block * 512, num_blocks * 512);
            return 0;
        }
        else
        {
...
int block_write(struct block_device *blk, void *buf, sector_t block, blkcnt_t num_blocks)
{
	if (fuzz_start)
    {
        if(((block + num_blocks) * 512) <= payload_buffer->size)
        {
            memcpy(payload_buffer->data + block * 512,buf,  num_blocks * 512);
            return 0;
        }
        else
        {
