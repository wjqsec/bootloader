...
extern kAFL_payload *payload_buffer;
...
{
	static int fuzz_used = 0;
    if((fuzz_used + 4) >= payload_buffer->size)
        kAFL_hypercall (HYPERCALL_KAFL_RELEASE, 0);
    uint32_t *frame_len = (uint32_t *)(payload_buffer->data + fuzz_used);
	if (*frame_len > 0xf0000000)
		kAFL_hypercall (HYPERCALL_KAFL_RELEASE, 0);
	if((fuzz_used + 4 + *frame_len) > payload_buffer->size)
        kAFL_hypercall (HYPERCALL_KAFL_RELEASE, 0);
	net_receive(edev, payload_buffer->data + fuzz_used + 4, *frame_len);
	fuzz_used += 4 + *frame_len;
    return 0;
