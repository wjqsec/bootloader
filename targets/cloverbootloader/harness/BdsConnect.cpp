...
EFI_HANDLE collected_handlers[100];
int num_handlers=0;
		...
		if (Device) {					
			...
			if (!Parent) {
				if (HandleType[Index] & EFI_HANDLE_TYPE_DEVICE_HANDLE) {
					...
					// Status = gBS->ConnectController(AllHandleBuffer[Index], NULL, NULL, true);
                    collected_handlers[num_handlers++] = AllHandleBuffer[Index];
				}
			}
		}
...
VOID ConnectHandlers(VOID)
{
    // for(int i = 0 ; i < 2 ; i++)
    // {
    //     gBS->ConnectController(collected_handlers[i], NULL, NULL, true);
    // }
    gBS->ConnectController(collected_handlers[2], NULL, NULL, true);
}
